...
- **Converge the half-migrated warning prefix** — slice 2 introduced the `[warn]` role at the `config/*` broken-config sites, but ~5 CLI sites still emit raw `[yellow]Warning]` in two colon spellings (`[yellow]Warning[/yellow]` at `cli/_update_attach.py`, `cli/_status.py`; `[yellow]Warning:[/yellow]` at `cli/_merge.py`). Converge all warning prefixes onto the `[warn]` role (optionally via a `fmt_warning`-style path consistent with the roles migration) so the role isn't half-applied. Folded here from a slice-2 refactor deferral.
...
- `[warn]` must resolve to the same yellow the raw `[yellow]Warning]` sites render, so the warning-prefix convergence is visually identical.
...
- Warning-prefix sites to converge onto `[warn]`: `cli/_update_attach.py`, `cli/_status.py`, `cli/_merge.py`.
...
- No raw `[yellow]Warning...]` prefix remains in `src/`; all warning prefixes use the `[warn]` role, visually identical.
