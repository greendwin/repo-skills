...
def status(
    *,
    ...
    # every source in available_by_source comes from source_registry.sources, so the
    # registry already subsumes it; the union adds registered sources with no skills
    # plus manifest-lingering (unregistered-but-installed) sources
    all_sources = sorted(
